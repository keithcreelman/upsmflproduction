...
def update_contract_info_aav(contract_info: str, next_salary: int, old_salary: int, old_year: int) -> str:
    info = safe_str(contract_info)
    if not info or next_salary <= 0 or old_year <= 1 or next_salary == old_salary:
        return info
    repl = f"AAV {format_k(next_salary)}"
