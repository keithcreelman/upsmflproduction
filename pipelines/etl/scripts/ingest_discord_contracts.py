...
CONTRACT_ACTIVITY_FILE = str(ETL_ROOT / "inputs" / "discord_contract_activity.csv")
SLACK_HISTORY_FILE = str(ETL_ROOT / "inputs" / "discord_slack_history.csv")
...
def main():
    ...
    parser.add_argument(
        "--contract-activity-file",
        default=CONTRACT_ACTIVITY_FILE,
        help="Path to Discord contract activity export CSV",
    )
    parser.add_argument(
        "--slack-history-file",
        default=SLACK_HISTORY_FILE,
        help="Path to Discord archived/slack-history export CSV",
    )
    parser.add_argument(
        "--write-v3-all",
        action="store_true",
    ...
    parsed_rows = []
    parsed_rows.extend(parse_file(args.contract_activity_file, "contract-activity"))
    parsed_rows.extend(parse_file(args.slack_history_file, "slack-history"))
