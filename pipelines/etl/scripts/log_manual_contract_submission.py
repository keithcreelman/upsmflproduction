#!/usr/bin/env python3
"""
Append a Front Office manual contract submission record into manual_contract_submissions.json.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--json-path", default="manual_contract_submissions.json")
    ...
    parser.add_argument("--submission-kind", default=os.environ.get("SUBMISSION_KIND", ""))
    ...
    parser.add_argument(
        "--source",
        default=os.environ.get("SOURCE", "worker-front-office-manual-contract-update"),
    )
    parser.add_argument(
        "--submission-id",
...
def build_submission_id(entry: Dict[str, Any]) -> str:
    raw = "|".join(
        [
            ...
            safe_str(entry.get("submission_kind")),
...
def main() -> int:
    ...
    submission_kind = safe_str(args.submission_kind).replace("_", "-").lower()
    ...
    entry: Dict[str, Any] = {
        ...
        "submission_kind": required["submission_kind"],
        ...
        "source": safe_str(args.source) or "worker-front-office-manual-contract-update",
    }
    ...
    doc["meta"] = {
        ...
        "source": "manual-contract-submission-log",
    ...
    print(
        f"Logged manual contract submission for player {entry['player_id']} "
        f"({entry['player_name']})."
    )
