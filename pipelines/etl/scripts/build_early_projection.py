...
DEFAULT_TAG_TRACKING_JSON = os.getenv(
    "MFL_TAG_TRACKING_JSON", str(ETL_ROOT / "inputs" / "tag_tracking.json")
)
DEFAULT_TAG_EXCLUSIONS_JSON = os.getenv(
    "MFL_TAG_EXCLUSIONS_JSON", str(ETL_ROOT / "inputs" / "tagging_2026_exclusions.json")
)
DEFAULT_MANUAL_OVERRIDES_JSON = str(ETL_ROOT / "config" / "early_projection_2026_overrides.json")
DEFAULT_SLEEPER_SF_ADP_PATH = str(ETL_ROOT / "config" / "sleeper_sf_adp_2026_raw.txt")
...
def main():
    ...
    for r in roster:
        ...
        tag_salary_val = coerce_int(t.get("tag_salary")) or coerce_int(t.get("tag_bid")) or 0
        if tag_salary_val <= 0 and expiring:
            # Fallback to current salary so newly eligible players still surface with a value.
            tag_salary_val = coerce_int(r.get("salary")) or 0
    ...
    print(
        "Outputs: "
        f"{LOG_DIR / 'early_projection_2026_summary.csv'}, "
        f"{LOG_DIR / 'early_projection_2026_contract_rollover.csv'}, "
        f"{LOG_DIR / 'early_projection_2026_auction_pool_values.csv'}, "
        f"{LOG_DIR / 'early_projection_2026_expected_points.csv'}, "
        f"{LOG_DIR / 'early_projection_2026_team_cap.csv'}, "
        f"{LOG_DIR / 'early_projection_2026_topn.csv'}, "
        f"{LOG_DIR / 'early_projection_2026_manual_override_resolution.csv'}, "
        f"{LOG_DIR / 'early_projection_2026_sleeper_adp_resolution.csv'}"
    )
    print(
