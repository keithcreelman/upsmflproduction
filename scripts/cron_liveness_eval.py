...
def main() -> int:
    ...
    # check_reports is the whole point of report_gate(): False means the flags
    # POSITIVELY confirm FA reports are off (auction over), so "no report
    # today" is the expected state, not a problem — skip the slot checks
    # entirely. It was computed above and never consumed here; that gap is
    # exactly why the 08-08 page didn't stop when the auction ended. True
    # covers both "genuinely armed" and "unreadable, so stay armed and alert
    # via flag_note" (see the caveat appended below).
    for mode, hour in (REPORT_SLOTS if check_reports else ()):
