...
if rg -n "/Users/|Desktop/MFL_Scripts" "$ROOT_DIR/pipelines/etl/scripts" "$ROOT_DIR/services/rulebook" "$ROOT_DIR/services/mcm" "$ROOT_DIR/apps/mfl_site" "$ROOT_DIR/site"; then
  echo "Found disallowed absolute paths."
  exit 1
fi

echo "==> Checking for embedded API keys"
if rg -n "APIKEY=" "$ROOT_DIR/pipelines/etl/scripts" "$ROOT_DIR/services/rulebook" "$ROOT_DIR/services/mcm" "$ROOT_DIR/site"; then
