(function () {
  ...
  // MYM submit endpoint
  const OFFER_MYM_URL = "https://upsmflproduction.keith-creelman.workers.dev/offer-mym";
  const OFFER_RESTRUCTURE_URL =
    "https://upsmflproduction.keith-creelman.workers.dev/offer-restructure";
  ...
  function getLocalTestScopeId() {
    const host = safeStr(window.location.host || "");
    const league = safeStr(getResolvedLeagueId() || getLeagueId() || DEFAULT_LEAGUE_ID || "");
    return `${host}|${league}`;
  }

  function loadScopedLocalTestRows(storageKey) {
    try {
      const raw = localStorage.getItem(storageKey);
      if (!raw) return [];
      const parsed = JSON.parse(raw);
      if (parsed && typeof parsed === "object" && !Array.isArray(parsed) && parsed.scoped) {
        const scopeId = getLocalTestScopeId();
        const scopedVal = parsed.scoped && parsed.scoped[scopeId];
        const rows = normalizeSubmissions(scopedVal);
        return rows.map(normalizeSubmissionRow);
      }
      const legacyRows = normalizeSubmissions(parsed);
      return legacyRows.map(normalizeSubmissionRow);
    } catch (e) {
      return [];
    }
  }

  function saveScopedLocalTestRows(storageKey, rows) {
    try {
      const scopeId = getLocalTestScopeId();
      let scoped = {};
      const raw = localStorage.getItem(storageKey);
      if (raw) {
        ...
        if (
          ...
          parsed.scoped &&
          typeof parsed.scoped === "object"
        ) {
          scoped = { ...parsed.scoped };
        }
      }
      scoped[scopeId] = { rows: Array.isArray(rows) ? rows : [] };
      localStorage.setItem(storageKey, JSON.stringify({ scoped }));
    } catch (e) {}
  }

  function loadLocalTestMymSubmissions() {
    return loadScopedLocalTestRows(LOCAL_TEST_MYM_SUBMISSIONS_KEY);
  }

  function saveLocalTestMymSubmissions(rows) {
    saveScopedLocalTestRows(LOCAL_TEST_MYM_SUBMISSIONS_KEY, rows);
  }

  function loadLocalTestRestructureSubmissions() {
    return loadScopedLocalTestRows(LOCAL_TEST_RESTRUCTURE_SUBMISSIONS_KEY);
  }

  function saveLocalTestRestructureSubmissions(rows) {
    saveScopedLocalTestRows(LOCAL_TEST_RESTRUCTURE_SUBMISSIONS_KEY, rows);
  }
  ...
  function canRestructureRow(row) {
    ...
    const flag = row.restructure_eligible;
        ...
        const tagBtn = canTag
          ...
          <div class="ccc-tagActions">
            ...
            ${
              ...
              data-tag-unsubmit="1"
              data-tag-key="${htmlEsc(activeTagKey)}"
  ...
  function getActionCandidates(viewModel) {
    ...
    if (actionType === "restructure") {
      ...
      return sortCandidates(seasonRows.filter((r) => {
        ...
        const fid = pad4(r.franchise_id);
        if (!state.commishMode && (usageByTeam.get(fid) || 0) >= RESTRUCTURE_CAP_PER_TEAM) {
          return false;
        }
        return true;
      }));
  ...
  function buildActionPreview(actionType, row) {
    ...
    if (actionType === "mym") {
      ...
      return {
        ...
        statusHint: "Review and confirm your MYM contract.",
      };
    }
    if (actionType === "restructure") {
      ...
      return {
        ...
        submitEnabled: true,
        submitLabel: "Review",
        statusHint: "Review and confirm your restructure submission.",
      };
    }
    if (actionType === "tag") {
      const season = getUiSeasonForRow(row);
  ...
  function ensureActionFlowDefaults(actionType, candidates) {
    ...
    if (!selected) {
      ...
      state.actionFlow.statusMessage = "";  /* let statusHint carry the guidance */
    }
  }
  ...
  function renderActionPanel(viewModel) {
    /* Use state.actionFlow.actionType directly — do NOT fall back to viewModel.defaultActionType
       so that the placeholder state is preserved when no action has been chosen. */
    const actionType = normalizeActionType(state.actionFlow.actionType);
    const options = getActionTypeOptions();
    const candidates = actionType ? getActionCandidates(viewModel || {}) : [];
    if (actionType) ensureActionFlowDefaults(actionType, candidates);
    const selectedRow = actionType ? findActionSelectedRow(candidates) : null;
    const preview = actionType ? buildActionPreview(actionType, selectedRow) : { current: [], proposed: [], changes: [], submitLabel: "", statusHint: "", submitEnabled: false };
    ...
    const actionDropdownOptions = options
      .map((opt) => {
        ...
        const disabled = !!opt.disabled || isPlaceholder;
        const suffix = opt.disabled && !isPlaceholder ? " (Coming Soon)" : "";
        return `<option value="${htmlEsc(opt.type)}"${selected ? " selected" : ""}${disabled ? " disabled" : ""}${isPlaceholder ? " hidden" : ""}>${htmlEsc(opt.label)}${suffix}</option>`;
      })
      .join("");
    const actionDropdownHint = actionType ? getActionCardDesc(actionType) : "";

    /* Step 2 — Player List with inline search */
    const searchVal = safeStr(state.search || "");
    let playerRows;
    if (!actionType) {
      playerRows = `<div class="ccc-flowLocked">Select an action above to continue.</div>`;
      ...
      playerRows = candidates
        ...
        .map((r) => {
          ...
          if (actionType === "extend") {
            ...
            const side = getTagSideForRow(r, "OFFENSE");
            const fallbackKey = buildTagSelectionKey(dlSeason, r.franchise_id, side);
            ...
            if (isSelectedByTag || submittedForRow) {
              const pastDeadline = isTagDeadlinePassed(dlSeason);
              const unsubmitDisabled = submittedForRow && pastDeadline && !state.commishMode;
              const unsubmitTitle = unsubmitDisabled
                ? "Tag deadline has passed"
                : submittedForRow
                ? "Remove submitted tag"
          ...
          return `
            <div class="ccc-flowPlayerRow${isActive ? " is-active" : ""}${actionType === "tag" && isTagLocked ? " is-locked" : ""}">
  ...
  function renderReportsPanel(viewModel) {
    /* ── 1. Raw rows — always all-teams (getAllSubmissionRows has no team gating) ── */
    ...
    rows = rows.slice().sort((a, b) => {
      let va, vb;
      if (sortKey === "submitted_at_utc") {
        va = parseDate(a.submitted_at_utc) || new Date(0);
    ...
    /* Update selected */
    if (!safeStr(rv.selectedSubmissionId) && rows.length) {
      rv.selectedSubmissionId = safeStr(rows[0].id);
    }
    const selected = rows.find((r) => safeStr(r.id) === safeStr(rv.selectedSubmissionId)) || rows[0] || null;
    rv.currentPayload = selected ? selected.raw : null;

    /* ── 3. Report Type dropdown (independent of Action dropdown) ── */
    ...
    const rSortTh = (key, label, extra) => {
      ...
      const cls = ["ccc-rTh is-sortable", active ? "is-sorted" : "", extra || ""].join(" ").trim();
      return `<th class="${cls}" data-report-sort="${htmlEsc(key)}">${htmlEsc(label)}${icon}</th>`;
    };
    ...
    const detailsRows = selected
      ? `
        <div class="ccc-reportDetailsGrid">
          ...
          <div><span>Contract</span><strong>${htmlEsc(selected.contract_status || "—")}</strong></div>
      ...
      `
      : `<div class="ccc-flowEmpty">Select a row to inspect details.</div>`;
    ...
    return `
      <div class="ccc-modeCard ccc-modeCard-reports">
        ...
        <div class="ccc-modeGrid">
          <div class="ccc-modeStat"><div class="label">Showing</div><div class="value">${rows.length}</div></div>
          <div class="ccc-modeStat"><div class="label">Total</div><div class="value">${allRows.length}</div></div>
          <div class="ccc-modeStat"><div class="label">Ext Eligible</div><div class="value">${safeInt(health.eligibleExtensions || 0)}</div></div>
          <div class="ccc-modeStat"><div class="label">Rst Eligible</div><div class="value">${safeInt(health.eligibleRestructures || 0)}</div></div>
        </div>

        <section class="ccc-flowSection ccc-reportTableSection">
          <div class="ccc-flowTitle">Submissions</div>
          <div class="ccc-reportTableWrap">
            <table class="ccc-reportTable">
              <thead>
                <tr>
  ...
  function actionTypeForModule(moduleKey) {
    ...
    /* Empty/unknown module — return current actionFlow type without forcing a default */
    return normalizeActionType(state.actionFlow.actionType);
  }
  ...
  function applyFiltersForModule(moduleKey) {
    ...
    const forceOwnerDefaultTeam =
      !state.commishMode && (key === "mym" || key === "restructure" || key === "expiredrookie");
    if (forceOwnerDefaultTeam) {
      defaultTeam = ownerOrAll;
    } else if (state.commishMode && !safeStr(defaultTeam)) {
      defaultTeam = "__ALL__";
    }
  ...
  function getActiveTagSelectionForSide(season, franchiseId, side) {
    const normalizedSide = normalizeTagSideValue(side) || "OFFENSE";
    const rows = getTagSelectionsForTeam(season, franchiseId).filter((sel) => {
      const rowSide =
        normalizeTagSideValue(
          sel && (sel.side || sel.tag_side || getTagSideFromPos(sel.pos || sel.position))
        ) || "OFFENSE";
      return rowSide === normalizedSide;
    });
    ...
    rows.sort((a, b) => rankTime(b) - rankTime(a));
    return rows[0] || null;
  }
  ...
  function render() {
    ...
    if (!state.activeModule) {
      /* In action-first mode, no active module just means no action chosen yet —
         show the primary tabs + action panel with the placeholder/locked state.
         Only the legacy tab/table UI gets hidden. */
      ...
      /* Fall through to render action panel with empty/locked state */
    }
    ...
    const eligibleRowsRaw =
      state.activeModule === "restructure"
        ? scopedEligibility.filter((r) => {
            ...
            const fid = pad4(r.franchise_id);
            if (!state.commishMode && (restructureUsageByTeam.get(fid) || 0) >= RESTRUCTURE_CAP_PER_TEAM) {
              return false;
            }
            return true;
          })
  ...
  function openDevNotice() {
    ...
    if (bodyEl) {
      bodyEl.textContent = canLive
        ? "Go-live is expected no later than 3/1/2026. Expect ongoing website changes while development continues. Your session has franchise 0008 access, so live submissions are enabled. Other franchises remain test-mode only."
        : "Go-live is expected no later than 3/1/2026. Expect ongoing website changes while development continues. You can submit contracts in test mode only until launch. Live submissions are restricted to franchise 0008 for now.";
    }
  ...
  async function load() {
    try {
      ...
      const localTestMymRows = filterRowsToLeague(loadLocalTestMymSubmissions(), activeLeagueId, false)
        .filter((r) => safeStr(r && r.source).toLowerCase().includes("local-test"));
  ...
  function wireEvents() {
    ...
    document.addEventListener("change", (e) => {
      const sel = e.target && e.target.closest ? e.target.closest("[data-report-type-select='1']") : null;
      if (!sel) return;
      state.reportsView.reportType = safeStr(sel.value || "all");
      state.reportsView.selectedSubmissionId = "";
      render();
    }, true);
    ...
    /* Reports: global search input */
    (function () {
      let reportSearchTimeout;
      document.addEventListener("input", (e) => {
        const inp = e.target && e.target.closest ? e.target.closest("[data-report-search='1']") : null;
        if (!inp) return;
        clearTimeout(reportSearchTimeout);
        reportSearchTimeout = setTimeout(() => {
          state.reportsView.search = safeStr(inp.value || "").trim();
          state.reportsView.selectedSubmissionId = "";
          render();
        }, 200);
      }, true);
    ...
    /* Reports: sortable column header click */
    document.addEventListener("click", (e) => {
      const th = e.target && e.target.closest ? e.target.closest("[data-report-sort]") : null;
      if (!th) return;
      const key = safeStr(th.getAttribute("data-report-sort") || "");
      if (!key) return;
      if (state.reportsView.sortKey === key) {
        state.reportsView.sortDir = state.reportsView.sortDir === "asc" ? "desc" : "asc";
      } else {
        state.reportsView.sortKey = key;
        state.reportsView.sortDir = "asc";
      }
      render();
    }, true);
