(function () {
  ...
  // HARD SCOPE: only run if the HPM explicitly placed <div id="teamOpsMount"></div>
  // in the DOM. Prevents this module from rendering on pages where the HPM snippet
  // isn't present (e.g., Front Office, Scoreboard).
  ...
  function ensureMount() {
    // Never creates a new mount. The HPM owns the mount div; if it's gone,
    // the module silently no-ops. This prevents cross-page bleed.
    return document.getElementById("teamOpsMount");
  }
  ...
  function boot() {
    // Double-check the scope gate at boot time in case DOM ready fired before
    // the HPM's mount div was parsed (paranoid guard).
