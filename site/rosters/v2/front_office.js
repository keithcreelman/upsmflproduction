...
(function () {
  ...
  // Unix seconds → the ISO calendar day in Eastern time. Mirrors
  // site/m/app.js isoEtDayFromUnix — same convention every UPS deadline uses.
  function isoEtDayFromUnixFO(unix) {
    try {
      const parts = new Date(unix * 1000).toLocaleDateString("en-CA", { timeZone: "America/New_York" });
      return /^\d{4}-\d{2}-\d{2}$/.test(parts) ? parts : "";
    } catch (e) { return ""; }
  }
  // Live waiver/FA acquisitions straight from MFL's own transaction log —
  // the same override mobile already ships (site/m/app.js
  // fetchWaiverAcquisitionIndex / acquisitionForPlayer, "the reported bug").
  // player_acquisition_lookup_<yr>.json (URL_ACQUISITION_LOOKUP above) is a
  // commish-maintained snapshot regenerated by hand — this file's copy was
  // built 2026-03-10 and therefore cannot know about anyone dropped and
  // re-claimed since. Trey Benson: traded to 0008 2025-11-27 (the static
  // row, still the only one the snapshot has), dropped 2026-07-20, blind-bid
  // reclaimed by 0008 2026-08-07 — a genuine pre-deadline waiver pickup that
  // should walk the MYAC ladder. Without this override,
  // isPreseasonWwPickupFO's trade-exclusion check (line ~1908) reads the
  // stale "Trade" label and hides MYAC entirely. Mobile fixed this by
  // merging live transactions in and letting the newer record win; desktop
  // never got the same fix. Keyed fid:pid so an add by a PREVIOUS owner is
  // never read as the current owner's acquisition.
  async function loadLiveWaiverAcquisitions(year) {
    try {
      const j = await fetchJSON(apiUrl("/api/mfl-export") + "?TYPE=transactions&L=" +
        encodeURIComponent(LEAGUE_ID) + "&YEAR=" + encodeURIComponent(year) + "&JSON=1");
      if (!j || !j.transactions) return {};
      let rows = j.transactions.transaction;
      if (!Array.isArray(rows)) rows = rows ? [rows] : [];
      const map = Object.create(null);
      rows.forEach((t) => {
        if (!t) return;
        const type = safeStr(t.type).toUpperCase();
        let label;
        if (type === "BBID_WAIVER") label = "WW blind-bid waiver claim";
        else if (type === "FREE_AGENT") label = "WW free-agent add (FCFS)";
        else return;
        const fid = pad4(t.franchise);
        const unix = safeInt(t.timestamp, 0);
        if (!fid || unix <= 0) return;
        const added = safeStr(t.transaction).split("|")[0];
        if (!added) return; // drop-only row
        const iso = isoEtDayFromUnixFO(unix);
        if (!iso) return; // unreadable → no entry, not a guess
        added.split(",").forEach((raw) => {
          const pid = safeStr(raw).replace(/\D/g, "");
          if (!pid) return;
          const key = fid + ":" + pid;
      ...
      console.warn("[fo] live waiver acquisition load failed (falling back to static lookup only):", e.message || e);
      return {};
    }
  }
  ...
  function mergeAcquisitionLookupRows(teams, rows, liveMap) {
    if (!teams || !teams.length) return;
    const hasLive = !!(liveMap && Object.keys(liveMap).length);
    if ((!rows || !rows.length) && !hasLive) return;
    ...
    for (let t = 0; t < teams.length; t += 1) {
      ...
      for (let p = 0; p < players.length; p += 1) {
        ...
        const key = acquisitionLookupKey(player.fid, player.id);
        ...
        // Newest wins: MFL's live transaction log (always current) vs the
        // static snapshot above (fresh only as of its last hand rebuild).
        // A trade recorded after a waiver claim is the later acquisition and
        // must take precedence, and vice versa — comparing ISO dates is
        // enough. See loadLiveWaiverAcquisitions for why this exists.
        const live = hasLive ? liveMap[key] : null;
        if (live && (!match || safeStr(live.date) >= safeStr(match.acquisition_date || match.date_et))) {
          player.acquisitionDate = safeStr(live.date);
          player.acquisitionDateTime = "";
          player.acquisitionTypeLabel = safeStr(live.label);
          player.acquisitionDetail = "";
          player.acquisitionUnix = safeInt(live.unix, 0);
        }
...
// ── Player bundle cache (Contract History, Stats, News) ──────────────
  ...
  function isPreseasonWwPickupFO(p) {
    ...
    // Exact instant when mergeAcquisitionLookupRows resolved this player off
    // the live transaction log (loadLiveWaiverAcquisitions); otherwise noon
    // ET of the acquisition day, the same fallback convention this file uses
    // elsewhere (extensionDeadlineForPlayer's acqDate, the mymDays acqDateMs
    // in rosterContractEligibility below) — needed for a claim made on
    // kickoff day itself, where the exact time is what settles it.
    var acqMs = safeInt(p && p.acquisitionUnix, 0) > 0 ? p.acquisitionUnix * 1000 : (function () {
      try {
  ...
  async function loadRosterData() {
    ...
    try {
      ...
      const [acqRows, liveAcqMap, _historyLoaded, mflAdj, nextSeasonAdj] = await Promise.all([
        loadAcquisitionLookup(SEASON),
        loadLiveWaiverAcquisitions(SEASON),
        loadRookieDraftHistory(),
        loadMflSalaryAdjustments(SEASON),
        loadNextSeasonDropPenalties(SEASON)
      ]);
      mergeAcquisitionLookupRows(STATE.teams, acqRows, liveAcqMap);
