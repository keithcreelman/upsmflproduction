...
export const FEATURE_FLAGS = [
  ...
  { key: "AUCTION_ERA_ENABLED", label: "ERA auction live", help: "ON = the Expired-Rookie Auction is running — the ERA tab goes live (board + bidding) on the app + desktop. OFF = the ERA tab shows a read-only eligible pool. Flip ON when ERA opens (Memorial Day weekend). ALSO gates the */5 auction poller: with BOTH this and AUCTION_ERA_ENABLED off, the poll no-ops (that is what keeps it from burning the D1 read budget year-round). Data is not lost — MFL's transaction log is append-only and re-read in full, so flipping back ON catches up. Force a poll regardless with POST /admin/auction/poll-now?force=1." },
  { key: "AUCTION_FAA_ENABLED", label: "FAA auction live", help: "ON = the Free-Agent Auction is running — the FAA tab goes live (board + bidding) on the app + desktop. OFF = the FAA tab shows a read-only available pool. Flip ON when the FA Auction opens (last weekend of July). ALSO gates the */5 auction poller: with BOTH this and AUCTION_ERA_ENABLED off, the poll no-ops (that is what keeps it from burning the D1 read budget year-round). Data is not lost — MFL's transaction log is append-only and re-read in full, so flipping back ON catches up. Force a poll regardless with POST /admin/auction/poll-now?force=1." },
  ...
  { key: "TAXI_CALLUP_WEEKLY_SYNC_ENABLED", label: "Taxi call-up weekly count (writes call-up weeks)", help: "ON = once a day (13:05 UTC) the worker counts every COMPLETED NFL week a taxi-eligible rookie (UPS Rookie Draft Round 2+, in the 3-year window, still on his Rookie-Draft contract, not permanently promoted) spent on his team\u2019s active roster, straight from MFL\u2019s weekly results, and records one call-up per week (canon \u00a7B2: 3 allowed, the 4th makes it permanent). This is what the Front Office \u2018Taxi Eligibility\u2019 row and the promote dialog count. OFF = nothing is counted and 2026 weeks show as unused. Preview any time: POST /admin/taxi-callups/sync-weeks?dry_run=1&APIKEY=\u2026", danger: true },
  { key: "WW_CONTRACT_STAMP_ENABLED", label: "WW contract stamp (writes the real 1-yr waiver contract)", help: "ON = after a waiver/FCFS award lands, write the canonical 1-year WW contract to MFL — contractStatus Vet-WW (Rookie-WW for an NFL rookie), contractYear 1, salary = the winning bid, contractInfo “CL 1| TCV {n}K| AAV {n}K”. MFL sets ONLY salary on an award and leaves the other three blank forever (its league default row is empty), which is why waiver pickups read as “status pending” and why the Front Office had nothing real to show. Unlike the cosmetic annotator this IS a money-path write, so it ships dark. It writes ONLY where all three of contractStatus/contractYear/contractInfo are empty — a contract MFL or an owner has described is never touched — and any award whose bid it cannot establish from MFL is reported as needs-input instead of defaulted. Preview any time: POST /admin/adds/stamp-ww-contracts?dry_run=1&APIKEY=…", danger: true },
  { key: "ERA_AUTODROP_ENABLED", label: "ERA expired-rookie auto-drop sweep (real MFL drops)", help: "ON = the hourly cron may run the once-per-season sweep that unloads expired-rookie contracts off rosters at the tag deadline. This is the most destructive automated path in the worker: it removes players from rosters, bypassing the cap-penalty math and the §A3 ERA retention gate. It re-fired on 2026-08-06 and unloaded Levis/Bigsby/Charbonnet — three ERA players who had been re-won by the same franchise that lost them — because its idempotency gate failed open on a D1 read error. That gate now fails closed, every candidate is revalidated against its LIVE contract before any drop, and this switch is the manual stop. OFF = the sweep refuses to run; the manual admin route still works. Leave OFF unless a tag-deadline sweep is actually due.", danger: true },
  { key: "IR_NATIVE_ANNOUNCE_ENABLED", label: "IR native-move announcer (real Discord posts)", help: "The IR placed/activated announcement only ever fired at write-time, from this app's own roster-action UI — a move made on MFL's native page was silent forever (found 2026-08-31: Hammer placed Josh Jacobs on IR with zero announcement). The hourly cron now diffs the live roster against ups_ir_roster_status and detects any non-IR↔IR transition, from either path. OFF = the diff still runs and the ledger stays warm every hour, but every detected transition is posted as dry_run only — nothing hits Discord. ON = a detected transition posts for real, using the exact same copy/GIF pools/thread structure as the app's own write-time announcement. Armed 2026-08-31 via a direct D1 override ahead of this registration shipping — this entry exists so the switch is visible and toggleable from the settings panel like every other kill switch, not just from a raw D1 write.", danger: true },
];
