...
export const FEATURE_FLAGS = [
  ...
  { key: "AUCTION_FAA_PENALTIES_ENABLED", label: "FAA missed-nom FINES (real money)", help: "ON = the 9 AM close books §F RULE 2 fines LIVE to the ledger ($3K/$7K/$15K escalating; every fine hits this season AND next — the next-season half stays ledger-only until the rollover). NOTE: nothing auto-posts to MFL yet — applying the current-season salaryAdj on MFL is a manual commish step today. OFF (test mode) = misses are still named in the report and recorded, but PRE-VOIDED — they never count toward the offense ladder and no money is owed. Keep OFF until the real auction opens; excuse/restore any day from the ⚖️ Missed Nominations tab.", danger: true },
