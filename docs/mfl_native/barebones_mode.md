...
**UPS Center exit (2026-09-17).** UPS Center (`?MODULE=MESSAGE19&hub=wire`) has no stock-MFL version, so in Lite Mode its page was blank. The guard now switches a *stored* Lite Mode preference OFF (writes `0`) when that page is opened, and the page renders in the full site. It is the same exit the player modal's "Open in Front Office (full site) →" link takes. An explicit `?ups_barebones=1` / `#ups_barebones=1` on the URL still wins. Other hubs are unchanged.
