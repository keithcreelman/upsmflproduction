...
- **Rounds 2–6** — 3-year contract, **taxi-squad eligible for first 3 LEAGUE years** (NOT NFL service time — see B2). Can stay on active roster instead. Only **Round 1 is taxi-ineligible**; every other round (2 through 6) is eligible.
- **Round 6 (UPDATED 2025+)** — Must be used to select **IDP only**. **Kickers and Punters are NOT eligible** (the prior PK/PN expansion was reversed in 2025). Pick is **NOT tradeable** (forces every team to make at least one IDP selection per year). Player can be traded after the pick is made. 3-year contract. Random draft order.
  - **Taxi eligibility (RATIFIED Keith 2026-07-17):** a Round 6 IDP **IS taxi-squad eligible**, exactly like Rounds 2–5 (per §B2 "Round 2 or later" / `draft_round >= 2`). This corrects the prior "Rounds 2–5" phrasing here **and** the code's stale `round <= 5` cap (worker `isTaxiEligible` + the mobile `isTaxiEligibleFor` mirror), which was wrongly benching R6 rookies (e.g. A.J. Haulcy, a 2026 R6.07 safety).
...
- **Penalty timing (3 buckets — unchanged):**
  ...
  - **Enforcement (added 2026-08-08).** Until this date nothing in the code decided *which cap year* a penalty landed on — `_computeDropPenalty` answered only "how much", so **every** penalty hit the season it was computed in. That put The Long Haulers' 2026-08-08 drops (Konata Mumpfield, KeAndre Lambert-Smith, $1K each) on the 2026 cap instead of 2027. The boundary instant is now resolved from the commish-maintained League Calendar (`ups_settings 'auction_calendar'` → `faa_open_at`) by `_faaAuctionStartUnix`, and the bucket by `_dropPenaltyCapSeason` (`worker/src/index.js`). A **next-season** penalty is **ledger-only** — `ups_drop_events.applies_to_season` (migration 0125), never written to the current season's MFL cap — exactly like the §F RULE 2 next-season fine below. **No fail-open:** if the auction start or the drop timestamp cannot be resolved, `POST /admin/drops/post-mfl` refuses the row rather than defaulting to the current season. Read-only preview: `GET /admin/drops/cap-season-preview`.
...
- **CAVEAT — reverse: re-engagement forfeits the roster-legal floor waiver (Keith 2026-07-29, binding).** §A2's floor waiver (below: the 2/day requirement is waived once a franchise can field a legal lineup) is not a license to keep actively bidding while sitting out nominations. Keith: *"if you fail to nominate then you're supposed to either bail out or you can finish your current open auctions but if you just jump back in without a nomination the day prior then it's the same as non compliance."* So after a missed day, staying excused requires either (a) going fully passive — no new bids, no new nominations — or (b) letting only already-open lots you're already in resolve on their own. Placing any **new** bid or nomination without having cured first **forfeits the waiver** — the missed day counts for real, exactly like a franchise with an actual roster deficit that skipped its noms. This is a commish judgment call, not an automatic derivation (same reasoning as the 4th-offense league-fit review above) — booked via `flagReengagementMiss()` / `POST /admin/auction/flag-reengagement-miss`, never by `closeEtDay` itself. Precedent: Hawks (fid 0012) missed nominating 2026-07-28 (roster was legal, would otherwise have been waived), then on 2026-07-29 placed a new bid (Rachaad White) and two new nominations without curing — booked as their 1st offense, $3K current-season + $3K 2027 ledger-only, posted live to MFL.
...
- **Dynasty Pot mechanism clarified (2026-08-15):** the $50/team annual contribution is NOT one lump pot with its own separate payout — it splits 50/50. Half ($25/team = $300/year) accrues toward the 3-year cumulative prize (unchanged: $900 paid out once per 3-year cycle to the best 3-year All-Play%). The other half ($25/team = $300/year) is added directly to that year's actual Champion and 2nd-place payouts — same winners as always, just a bigger check, not a separate leaderboard. This resolves the "$300 to 1st / $100 to 2nd, open to adjustment" note from the 2026-05-11 vote (`docs/league_context_changelog.md`): the real split, confirmed against that proposal's own arithmetic ($900 paid out annually over 3 years = $300/year), is $200 to Champion / $100 to 2nd — reduced to $180/$80 as of the restructuring below.
- **Payout restructuring (2026-08-15, commissioner decision — not a Discord vote, same precedent as the 2022-08-12 payout overhaul above, which also carries no vote tally):** Division winners $50 → $75 (4 × $75 = $300, up from $200). Site fee + AI credits merged into one **Site & Technology Fees** line ($130/year: $93.11 real 2026 MFL invoice + $36.89 provisional AI-credit budget for the Discord trade-roast bot; any unspent balance rolls forward to reduce next year's line). **Hawktuah Bowl formalized as a residual, not a fixed prize** — it always was (2025 precedent: $86.58, never written down); formula is Annual pool + Dynasty Pot transfer − Champion − 2nd − 3rd − Division Champions − Weekly High Score − Site & Technology Fees. For 2026 that's **$0.00**, fully absorbed into the Division bump. Funding: $60 from what would have gone to Hawktuah Bowl, $40 from trimming the Dynasty Pot annual boost (see bullet above).
